...
class ImpactPlugin {
	...
	private $middleman_url = 'https://saasler-woocommerce-impact-stg.herokuapp.com';
	...
	public function impact_handle_callback( $request ) {
		...
		if ( 201 === intval( $response['response']['code'] ) ) {
			add_option( 'impact_request_value', $consumer_key );
		}
	}
	...
	public function impact_settings_create_admin_page() {
		...
		if ( $bearer ) {
			...
			include plugin_dir_path( __FILE__ ) . 'includes/no_woocommerce_access.php';
		}
	}
	...
	public function impact_settings_sanitize( $input ) {
		...
		foreach ( $input as $key => $value ) {
			...
			if ( empty( $value ) && 'custom_script_5' !== $key ) {
				$e_value = $key;
				switch ( $key ) {
	...
	public function impact_account_sid_0_callback() {
		...
		printf(
			...
			isset( $value['impact_account_sid_0'] ) ? esc_attr( '********' . substr( $value['impact_account_sid_0'], -4 ) ) : ''
		);
	}
	...
	public function impact_auth_token_1_callback() {
		...
		printf(
			...
			isset( $value['impact_auth_token_1'] ) ? esc_attr( '********' . substr( $value['impact_auth_token_1'], -4 ) ) : ''
		);
	}
	...
	public function program_id_2_callback() {
		$value = get_option( 'impact_settings_option_name' );
		printf(
			'<input class="regular-text" type="text" name="impact_settings_option_name[program_id_2]" id="program_id_2" value="%s" required><small class="form-text text-muted">
	...
	public function event_type_id_3_callback() {
		$value = get_option( 'impact_settings_option_name' );
		printf(
			'<input class="regular-text" type="text" name="impact_settings_option_name[event_type_id_3]" id="event_type_id_3" value="%s" required><small id="passwordHelpBlock" class="form-text text-muted">
	...
	public function custom_script_5_callback() {
		$value = get_option( 'impact_settings_option_name' );
		printf(
			'<textarea class="large-text" rows="5" name="impact_settings_option_name[custom_script_5]" id="custom_script_5" >%s</textarea><small class="form-text text-muted">
...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ), true ) ) {
	...
	if ( class_exists( 'ImpactPlugin' ) ) {
		...
		register_deactivation_hook( __FILE__, array( $impact_plugin, 'deactivate' ) );
	}
}
